...
loginBtn.onclick = () => browser.runtime.sendMessage({ type: "AUTH_LOGIN" });

logoutBtn.onclick = () => {
  updateAuthUI({ authenticated: false }); // instant UI update
  browser.runtime.sendMessage({ type: "AUTH_LOGOUT" });
};

browser.runtime.sendMessage({ type: "AUTH_STATUS" }, updateAuthUI);
...
function load() {
  browser.runtime.sendMessage({ type: "GET_STORED" }, (resp) => {
    render(resp?.data || []);
  });
}

function clearStored() {
  browser.storage.local.set({ [STORAGE_KEY]: [] }, load);
}
